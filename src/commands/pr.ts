...
import { GitProvider, getBranchNameFromPR, parsePrReference, assertUrlMatchesRepo } from "../utils/pr.js";
...
export async function prWorktreeHandler(
    ...
    try {
        ...
        if (!prNumber) {
            ...
            // Accept the same reference forms as `wt open`: `#123`, `123`, or a
            // full PR/MR URL. URLs are validated against the current repository
            // before doing any work, then reduced to their number.
            const prRef = parsePrReference(prNumber);
            if (!prRef) {
                throw new Error(
                    `"${prNumber}" is not a valid ${requestType} number or URL. ` +
                    `Use a number (e.g. 123), "#123", or a ${isPR ? 'PR' : 'MR'} URL.`
                );
            }
            if (prRef.url) {
